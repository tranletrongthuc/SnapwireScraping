...
def load_set_multiple_labels(detail_collection_dir, img_dir, only_dict=False):
    """
    Load Training Set
    ...
    # Define function for generating label
    def generate_label(indexes, n_classes):
        label = [0.0] * n_classes
    ...
    for file_info in file_infos:
        filenames.append(file_info['file_path'])
        label_indexes = [classes.index(kw) for kw in file_info['keywords']]
        label = generate_label(label_indexes, n_classes)
        labels.append(label)
