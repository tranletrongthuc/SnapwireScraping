...
class ResNet50(object):
    ...
    def __init__(self, config, n_classes):
        ...
        # Input Layer
        self.input_tensor = tf.compat.v1.placeholder(tf.float32, [None, self.image_size, self.image_size, self.n_channels])
    ...
    def load_data(self, filenames, labels):
        ...
        parse_fn = lambda f, l: _parse_function(f, l, self.n_channels, self.image_size, self.training)
    ...
    def inference(self):
        ...
        # Stage 2
        with tf.variable_scope('scale2', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        # Stage 3
        with tf.variable_scope('scale3', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        # Stage 4
        with tf.variable_scope('scale4', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        # Stage 5
        with tf.variable_scope('scale5', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        # Fully-Connected
        with tf.variable_scope('fc', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for epoch in range(n_epochs + 1):
                ...
                if epoch % 25 == 0:
                    ...
                    print("Epoch " + str(epoch) + " -> Saved to training/epoch" + str(epoch) + "/model.ckpt\n")
    ...
    def load_pred(self, folder):
        ...
        # Parsing Images
        parse_fn = lambda f: _parse_image(f, self.n_channels, self.image_size, self.training)
    ...
    def load_images_to_model(self, list_cropped_image_paths):
        # Calculating Batch Size
        batch = len(list_cropped_image_paths)
        logging.info("       FOUND {0} IMAGES TO PREDICT".format(batch))
        ...
        # Creates Iterator over the Prediction DataSet
        with tf.name_scope('predict-data'):
            predict_dataset = (tf.data.Dataset.from_tensor_slices(tf.constant(list_cropped_image_paths))
                               .map(parse_fn, num_parallel_calls=4)
                               .batch(batch))
    ...
    def restore_weight(self, weight_path, debug = False):
        global global_sess
        # self.inference()
        ...
        saver.restore(global_sess, weight_path)
        logging.info("Finished restoring weights")


    def predict(self, is_logging=False, debug=False):
        '''
        ...
        global_sess.run(self.predict_iterator_init_op)

        with tf.name_scope('predict'):
            predictions = softmax(self.logits)
            predictions = global_sess.run(predictions)

        # predictions = global_sess.run(softmax(self.logits))
        logging.info('Took: {0} seconds'.format(time.time() - start_time))
